...
def make_coffee(customer_name):
    ...
    sum(i * i for i in range(1000000))  # จำลองงานด้าน CPU-bound
...
def main():
    ...
    print(f"{ctime()} | [Main PID: {main_pid}] [Main TID: {main_tid}] === เริ่มระบบจองคิวลูกค้าแบบ Synchronous ===")

    start_time = time()
    start_cpu = process_time()  # เริ่มจับเวลา CPU

    # ลูปทำงานตามลำดับทีละคิว (ทีละคน)
    ...
    print(f"หน่วยความจำที่ใช้ (RAM) ที่ใช้: {mem_mb:.2f} MB")
