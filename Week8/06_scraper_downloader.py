...
async def link_scraper(queue: asyncio.Queue, page_urls: list):
    """Producer: สแกนหน้าลิงก์รูปภาพแล้วใส่ลง Queue"""
    ...
    for page in page_urls:
        ...
        # จำลองว่าพบรูปภาพ 2 รูปต่อ 1 หน้า
...
async def image_downloader(queue: asyncio.Queue, worker_name: str):
    """Consumer: ดึงลิงก์จาก Queue ทีละรายการมาดาวน์โหลด"""
...
async def main():
    ...
    # 2. รอให้ Producer ทำงานจนเสร็จ
    await producer_task

    # 3. รอให้ Consumer เคลียร์งานใน Queue จนหมด
    await queue.join()

    # 4. ส่ง None 1 ครั้ง เพื่อแจ้งให้ Downloader ตัวเดียวหยุดทำงาน
